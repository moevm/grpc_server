...
Запись root занимает ~5 минут. После завершения — отключить USB-C, подать питание.
...
### `fastboot` зависает на `< waiting for any device >`

Запускать с `sudo`. Устройство должно быть видно в `lsusb`.

### Обновление с 5.10 на 6.6

Подключиться по серийному порту (115200 baud), прервать загрузку U-Boot нажатием клавиши и выполнить:
