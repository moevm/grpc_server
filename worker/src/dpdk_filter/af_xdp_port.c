...
int af_xdp_port_init(const char* iface_name, uint16_t* port_id, struct rte_mempool* mbuf_pool) {
    ...
    snprintf(vdev_name, sizeof(vdev_name), "net_af_xdp_%s", iface_name);
    ret = rte_vdev_init(vdev_name, vdev_args);
...
void af_xdp_port_close(const char* iface_name ,uint16_t port_id)
{    
