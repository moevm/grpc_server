...
static struct rte_timer cache_save_tim_ip;
...
void init_ip_cache(void) {
  ...
  rte_timer_init(&cache_save_tim_ip);
  save_interval_cycles = rte_get_timer_hz() * 3600;

  rte_timer_reset(&cache_save_tim_ip, save_interval_cycles, PERIODICAL,
                  rte_lcore_id(), cache_save_timer_ip, NULL);
}

int lookup_ip_cache(const char *domain, struct node_cache_ip **return_node) {
  int ret = rte_hash_lookup_data(ip_hash, domain, (void **)return_node);
  ...
  if (ret >= 0 && *return_node) {
    ...
    if (age_seconds >= (*return_node)->ttl_seconds) {
      ...
      int ret_del = rte_hash_del_key(ip_hash, domain);
      if (ret_del < 0) {
        printf("[ERROR] Failed to deleting an obsolete hashtable value\n");
        return -ENOENT;
      }
      rte_free((*return_node)->key_ip);
...
void free_ip_cache(void) {
  ...
  int ret = rte_timer_stop(&cache_save_tim_ip);
  if (!ret) {
