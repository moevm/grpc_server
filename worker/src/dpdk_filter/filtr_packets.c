...
bool check_categories_with_lvl(struct requested_classification* req_clas, struct trust_categories_with_lvl categories_with_lvl[MAX_CATEGORIES_BY_TRUST_LVL]) {
  ...
  for (int i = 0; i < MAX_CATEGORIES; i++) {
    for (int j = 0; j < MAX_CATEGORIES; j++) {
        if (strcmp(req_clas->get_categories[j], categories_with_lvl[i].locked_by_trust_category) == 0 && req_clas->get_trust_level < categories_with_lvl[i].trust_lvl) {
          return false;
      }
    }    
  }    
...
bool main_filtring(struct requested_classification* req_clas, struct BASE_POLICY* policy, char domain[DOMAIN_MAX_LEN]) {
  ...
  if (check_categories(req_clas->get_categories, policy->locked_categories) == false) {
    printf("This site has a locked category");
    return false;
  }

  if (check_trust_level(req_clas->get_trust_level, policy->min_trust_level) == false) {
    printf("This site has a too small trust level");
    return false;
  }

  if (check_categories_with_lvl(req_clas, policy->categories_with_lvl) == false) {
    printf("This site blocked in accordance with 'trust categories with level'");
    return false;
  }
