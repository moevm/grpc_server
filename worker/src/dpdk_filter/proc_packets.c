...
#include "../../include/metrics_bridge.h"
...
void package_sending_decision(bool solution_is_send, struct rte_mbuf *pkt,
                              ...
                              const char *domain, const char *reason) {
  if (solution_is_send) {
    metrics_record_packet_passed();
  ...
  metrics_record_packet_dropped(reason ? reason : "blocked");
  if (domain && domain[0] != '\0') {
    metrics_record_domain_blocked(domain);
  }
...
void pakage_processing(struct net_port *port_in, struct net_port *port_out,
  ...
  for (int i = 0; i < nb_rx; i++) {
    ...
    metrics_record_packet_received();
    ...
    if (info_pac.domain[0] == '\0') {
      ...
      if (check_is_exception(info_pac.number_port) == true) {
        package_sending_decision(true, pkts[i], port_exception, queue_number,
                                 info_pac.domain, "exception_port");
        continue;
      }
      ...
      if (ret >= 0 && cached_node_ip) {
        ...
        if (ret >= 0 && cached_node) {
          package_sending_decision(
              cached_node->solution_is_send, pkts[i], port_out, queue_number,
              info_pac.domain,
              cached_node->solution_is_send ? NULL : "cached_blocked");
        } else if (ret == -ENOENT) {
          struct requested_classification req_clas;
          bool solution_is_send =
              main_filtring(&req_clas, policy, info_pac.domain);

          package_sending_decision(solution_is_send, pkts[i], port_out,
                                   queue_number, info_pac.domain,
                                   solution_is_send ? NULL : "filter_blocked");
          ...
          printf("[ERROR] Failed to search a key-value pair in the hash table: "
                 "%s\n",
                 strerror(-ret));
          struct node_cache_ip *new_node =
              rte_calloc("struct_node_cache_ip", 1,
                         sizeof(struct node_cache_ip), RTE_CACHE_LINE_SIZE);
          if (!new_node) {
        ...
        if (ret >= 0 && cached_node_domain) {
          package_sending_decision(cached_node_domain->solution_is_send,
                                   pkts[i], port_out, queue_number);
        } else if (ret == -ENOENT) {

          struct requested_classification
              req_clas; // query to domain controller
          ...
          struct node_cache_domain *new_node =
              rte_calloc("struct_node_cache", 1,
                         sizeof(struct node_cache_domain), RTE_CACHE_LINE_SIZE);
          if (!new_node) {
