...
def pars():
    parser = argparse.ArgumentParser(
        description="Traffic generator with RPS control and timeouts", formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument("--quantity", "-q", type=int, default=10, help="Number of requests (10 by default)")
    parser.add_argument(
        "--rps", "-r", type=int, default=15, help="The desired number of requests per second (15 by default)"
    )
    parser.add_argument("--timeout", "-t", type=int, default=20, help="Timeout per request in seconds (20 by default)")
    parser.add_argument(
        "--file", "-f", type=str, default="sites.txt", help="A file with a list of sites (by default sites.txt )"
    )
    parser.add_argument(
        "--max_concurrent", "-m", type=int, default=50, help="Maximum number of simultaneous tasks (50 by default)"
    )
    parser.add_argument(
        "--log_level",
        "-l",
    ...
    parser.add_argument(
        "--name_folder_log", "-n", type=str, default="logs", help="A name of folder logs(by default logs)"
    )
    ...
    if args.timeout <= 0:
        print(
            "Error: the timeout must be a positive number, using the default value of 20 - the optimal time for analyzing a compound is at standard values."
        )
        args.timeout = 20
    if args.max_concurrent <= 0:
        print(
            "Error: the number of tasks being completed at the same time must be a positive number, using the default value of 50"
        )
...
async def check_one(site, timeout):
    ...
    result = {"site": site, "tcp_ports": {}, "udp_ports": {}, "status": "unknown", "ip": None}

    try:
        nm = nmap.PortScanner()

        await asyncio.wait_for(
            asyncio.to_thread(nm.scan, site, "53,80,443,123,161", f"-sS -sU -T4 --host-timeout {scan_timeout}s"),
            timeout,
        )
                ...
                for port in nm[result["ip"]][proto].keys():
                    ...
                    if state == "open":
                        if proto == "tcp":
                            ...
                            result["tcp_ports"][port] = {"state": state, "service": service}
                            ...
                            result["udp_ports"][port] = {"state": state, "service": service}
...
def log(quantity, rps, timeout, max_concurrent, results, logger, file_log):
    ...
    log_data = {
        "parameters": {"quantity": quantity, "rps": rps, "timeout": timeout, "max_concurrent": max_concurrent},
        "results": [],
    }

    for res in results:
        if isinstance(res, Exception):
            log_data["results"].append({"site": "unknown", "status": "exception", "details": str(res)})
