...
Worker::Worker()
{
    ...
    std::cerr << "[INFO] Worker registered successfully. Ready to receive tasks." << std::endl;
}
...
void Worker::MainLoop()
{
    while (true) {
        ...
        if (client_fd < 0) {
            throw WorkerException("Failed to accept connection via communication socket");
        }
