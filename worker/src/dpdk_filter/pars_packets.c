...
void parsing_pakage(struct rte_mbuf* packet, struct info_of_pakage* info_pac) {
    ...
    if (pkt_type & RTE_PTYPE_L3_IPV4) {
        ...
        struct rte_ipv4_hdr* ipv4_hdr = rte_pktmbuf_mtod_offset(packet, struct rte_ipv4_hdr*, l3_offset);
        
        if (pkt_type & RTE_PTYPE_L4_TCP) {
            struct rte_tcp_hdr* tcp_hdr = rte_pktmbuf_mtod_offset(packet, struct rte_tcp_hdr*, l3_offset + hdr_lens.l3_len);
            info_pac->number_port = tcp_hdr->dst_port;
        } 
        else if (pkt_type & RTE_PTYPE_L4_UDP) {
            struct rte_udp_hdr* udp_hdr = rte_pktmbuf_mtod_offset(packet, struct rte_udp_hdr*, l3_offset + hdr_lens.l3_len);
    ...
    if ((pkt_type & RTE_PTYPE_L4_UDP) && info_pac->number_port == rte_cpu_to_be_16(53)) {
        uint32_t l4_offset = l3_offset + hdr_lens.l3_len + hdr_lens.l4_len;
        uint8_t* udp_payload = rte_pktmbuf_mtod_offset(packet, uint8_t*, l4_offset);
